...
######################################################################
# In this assignment, we'll be implementing a simple text analysis
# system, specifically designed to work with a corpus of
# presidential speeches.
#
# The Corpus() class represents a collection of speeches, each
# represented by a Speech object. The Corpus() object encapsulates
# comprehensive information about the corpus, including, e.g., overall
...
class Corpus():
    ...
    def updateFreqs(self, speech):
        for i in speech.wfreq: #Iterate through wfreq
            try: #Increment the frequencies unless the key is not in the dictionary
                self.dfreq[i] += 1 
                self.wfreq[i] += 1
            except: #If the key is not in the dictionary intialize it already incremented
                self.dfreq[i] = 1
                self.wfreq[i] = 1
        

    # Find the top k most frequently used words in the Corpus that do
    # not appear in every document. Based on code presented in
    # class. Note: if there is only one Speech in the Corpus, then
    # there will be no words in topK (because they all appear in all
    # of the Corpus documents).
    #
    # FIXME
    #
    def topK(self, k):
        #Create a list of words and find their freqeuncy if they aren't used in every speech
        l = [(i, self.wfreq[i]) for i in self.wfreq.keys() if self.dfreq[i] != len(self.speeches)]
        #Use swapping algorithm to sort the list using a temporary lambda function
        for i in range(k):
            j = i + l[i:].index(max(l[i:], key = lambda x: x[1]))
            l[i], l[j] = l[j], l[i] #Swap
        return([p[0] for p in l[:k]])#Return a list of the most k used words
    ...
    def createVectors(self, k):
        self.template = self.topK(k) #Create the template using topK function
        for i in self.speeches.keys(): #Iterate through the speeches
            #Create a vector for each speech and store it in the dictionary
...
class Speech():
    ...
    def __init__(self, filename):
        ...
        def expandWord(word):
            for x in REGEXP: #Iterate throgh regular expressions
                word = re.sub(x[0], x[1], word) #Complete sub if applicable
        ...
        #
        def expandText(text):
            for i in REGEXP: #Iterate through regular expressions
                text = re.sub(i[0], i[1], text) #Complete sub if applicable
        ...
        self.sentences = [s.strip() for s in self.text.split('.') if s != ''] #Strips the text into sentences
        ...
        self.words = expandText(self.text).lower().split(' ') #Removes regular expressions from the text after splitting
        ...
        # FIXME
        #
        self.wfreq = {} #Initialize frequency dictionary
        for i in self.words: #Iterate through words
            if i not in SW: #Checks if i (word) is not a stop word
                if i not in self.wfreq.keys(): #Checks if the key exist
                    self.wfreq[i] = 0 #Initialize a key if needed
                self.wfreq[i] += 1 #Increment the frequency
    ...
    def cosSimilarity(self, other):
        ...
        def vectorMagnitude(vector):
            #Creates a list of the numbers squared then adds them before computing the square root
            return(sqrt(sum([i**2 for i in vector])))

        
        # Compute the dot product and divide by the product of the
        # individual vector magnitudes.
        return(sum([ pair[0]*pair[1] for pair in zip(self.vector, other.vector) ]) / 
               (vectorMagnitude(self.vector)*vectorMagnitude(other.vector)))
